...
editor.setValue((localStorage.MINESWEEPER_savedCode || 'function onTick(field) {\n\t// TODO\n\t\n}'), -1);
if (localStorage.MINESWEEPER_caretPos) {
	try {
		let pos = JSON.parse(localStorage.MINESWEEPER_caretPos);
		editor.moveCursorTo(pos.row, pos.column);
	} catch (e) {}
}
editor.getSession().on('change', function () {
	localStorage.MINESWEEPER_savedCode = editor.getValue();
	localStorage.MINESWEEPER_caretPos = JSON.stringify(editor.getCursorPosition());
});
if (localStorage.MINESWEEPER_codeSize)
	editor.setFontSize(parseInt(localStorage.MINESWEEPER_codeSize));
editor.container.addEventListener('keydown', function (e) {
	if ((e.key == "=" || e.key == "-") && e.altKey) {
		...
		localStorage.MINESWEEPER_codeSize = editor.getFontSize();
	}
});
...
var activeSection = localStorage.MINESWEEPER_activeSection || "docs";
...
function setActiveSection(sectionName) {
	localStorage.MINESWEEPER_activeSection = sectionName;
